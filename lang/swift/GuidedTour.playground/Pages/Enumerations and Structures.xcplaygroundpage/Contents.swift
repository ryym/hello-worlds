...
//:
enum Rank: Int {
    case Ace = 1  // The default start value is zero.
...
compareRank(Rank.Two, rankB: Rank.Jack)
compareRank(Rank.King, rankB: Rank.Jack)
...
//:
// 値からenumに変換できるの便利! Javaだと毎回コンバータ書く必要あった。。
...
//:
// IMPORTANT: classは常に参照渡しだが、structureは常に値がコピーされる。
struct Card {
    var rank: Rank
    var suit: Suit
    func simpleDescription() -> String {
...
var tmpCard = threeOfSpades
tmpCard.rank = Rank.Ace
threeOfSpades.rank
...
//:
// Javaのenumはシングルトンだったから個々に別の値を持たせるなんてできなかったが、
// Swiftではそれができる。という事は、ServerResponse.Result(..)が呼ばれる度に
// 新しいインスタンスが作られているということ?

// Swiftのenumは、Scalaでいうcase classに近い感じ?
